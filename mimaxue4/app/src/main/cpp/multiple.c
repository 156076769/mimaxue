...
void bignum_write_file(bignum* b, FILE* fp) {
    int cap = 100, len = 0, i;
    char* buffer = malloc(cap * sizeof(char));
    bignum *copy = bignum_init(), *remainder = bignum_init();
    if(b->length == 0 || bignum_iszero(b)) printf("0");
 ...
 */
bignum *encodeMessage(int len, int bytes, char *message, bignum *exponent, bignum *modulus, FILE *fp) {
	...
	for(i = 0; i < len; i += bytes) {
		...
		printf(" ");
#endif
		bignum_write_file(&encoded[i/bytes], fp);
	}
	return encoded;
}
...
int main(void) {
	...
	printf("Opening file \"my_json_request.txt\" for reading\n");
	f = fopen("my_json_request.txt", "r");
	if(f == NULL) {
		printf("Failed to open file \"my_json_request.txt\". Does it exist?\n");
		return EXIT_FAILURE;
	}
	len = readFile(f, &buffer, bytes); /* len will be a multiple of bytes, to send whole chunks */
	
	printf("File \"my_json_request.txt\" read successfully, %d bytes read. Encoding byte stream in chunks of %d bytes ... ", len, bytes);
	//getchar();
	printf("\n");
	remove("my_json_request_enc9.txt");
    FILE *f91 = fopen("my_json_request_enc9.txt", "a");
	encoded = encodeMessage(len, bytes, buffer, e, n, f91);
	fclose(f91);
